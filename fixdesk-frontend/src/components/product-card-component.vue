...
<template>
  <div
    class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow duration-300 flex flex-col h-full group">

    <div class="h-48 w-full bg-gray-50 relative flex items-center justify-center overflow-hidden">

      <img v-if="product.imageUrl && !isImageError" :src="product.imageUrl" :alt="product.name"
    ...
    <div class="p-4 flex flex-col flex-grow">
      ...
      <div class="space-y-2 text-sm text-gray-500 mb-4 flex-grow">
        ...
        <div class="flex justify-between items-center pt-2 mt-2 border-t border-gray-100">
          ...
          <span class="px-2 py-1 rounded-md text-xs font-bold" :class="statusColorClass">
            {{ product.status }}
          </span>
        </div>
      ...
      <button @click="emit('add', product, imageRef)"
        class="w-full py-2.5 rounded-lg bg-blue-50 text-blue-600 font-medium hover:bg-blue-100 active:scale-95 transition-all duration-200 mt-auto">
        เพิ่มลงตระกร้า
      </button>
