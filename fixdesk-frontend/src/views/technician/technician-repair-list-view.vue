...
const rows = ref([
  [
    '11/11/1111', // วันที่
    'ABC-000', // หมายเลขแจ้งซ่อม
    'คอมพิวเตอร์เปิดไม่ติด', // หัวข้อแจ้งซ่อม
    'ABC-000', // หน่วยงาน
    'ชั้น 3 อาคาร 4 ห้อง 5', // สถานที่

    // ความเร่งด่วน (แดง "เร่งด่วนมาก")
    `<span class="inline-flex items-center justify-center h-8 font-medium text-red-600 bg-red-100 rounded-full w-28">
      เร่งด่วนมาก
    </span>`,

    // สถานะงาน (เหลือง "กำลังดำเนินการ")
    `<span class="inline-flex items-center justify-center h-8 font-medium rounded-full w-28 bg-amber-100 text-amber-700">
      กำลังดำเนินการ
    </span>`,

    'detail', // คอลัมน์ "รายละเอียด" → ปุ่ม i สีน้ำเงิน
    'actions', // คอลัมน์ "การจัดการ" → 3 ปุ่ม รับงาน/เปลี่ยนสถานะ/เสร็จสิ้น
  ],
  [
    '11/11/1111',
    'ABC-000',
...
async function fetchAllRepairs() {
  try {
    ...
    rows.value = data.map((r) => {
      ...
      const statusKey = r.rf_user_status // ค่าดิบของสถานะงาน

      return [
        new Date(r.rf_create_at).toLocaleDateString('th-TH'), // 0 วันที่
        r.rf_code, // 1 ใบแจ้งซ่อม
        `${r.us_first_name} ${r.us_last_name}`, // 2 ชื่อผู้แจ้ง
        r.department_name || '-', // 3 หน่วยงาน
        r.tt_name || '-', // 4 ประเภทงาน / สถานที่
        urgencyBadge, // 5 ความเร่งด่วน (badge สี)
        statusBadge, // 6 สถานะงาน (badge สี)
        statusKey, // 7 สถานะดิบ (pending / in_progress / done)
        'detail', // 8 → คอลัมน์ "รายละเอียด"
        'actions', // 9 → คอลัมน์ "การจัดการ"
      ]
    })
  } catch (err) {
...
<template>
  ...
  <div class="p-8 mx-auto bg-white shadow-md rounded-xl max-w-7xl">
    ...
    <!-- ปุ่มแจ้งซ่อม ยังไม่ได้ link ไปหน้าแจ้งซ่อม-->
    <button
      class="px-5 py-2 text-sm font-medium text-white bg-[#0072C3] rounded-[5px] shadow-sm hover:bg-[#005a9a] active:scale-[0.98] transition"
